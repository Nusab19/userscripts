...
(function () {
  ...
  function createProgressIndicator() {
    ...
    progress.style.cssText =
      "position: fixed; bottom: 1rem; right: 1rem; z-index: 9999; min-width: 200px;";
    progress.innerHTML = `
  ...
  function createDownloadButtons() {
    ...
    buttonContainer.style.cssText =
      "position: fixed; top: 1rem; right: 1rem; display: flex; gap: 0.5rem; z-index: 9999;";
    ...
    document
      ...
      .addEventListener("click", async () => {
        try {
          if (
            confirm(
              "This will fetch all exams from the server again. Continue?",
            )
          ) {
            ...
            const mainContainer =
              document.querySelector(".uu-main.p-0") ||
    ...
    document
      .getElementById("download-filtered")
      .addEventListener("click", () => {
        try {
          if (!filteredExams || filteredExams.length === 0) {
            alert(
              "No filtered exams to download. Please wait for data to load.",
            );
            return;
          }
          ...
          alert(
            "Failed to download filtered exams. Check console for details.",
          );
        }
      });
  ...
  async function fetchAllExams(forceRefresh = false) {
    ...
    console.log(
      forceRefresh
        ? "🔄 Force refreshing all exams..."
        : "Starting to fetch exams...",
    );
    ...
    while (hasMorePages) {
      try {
        ...
        const response = await fetch(
          "https://online.udvash-unmesh.com/Routine/LoadExamRoutineAjax",
          {
            method: "POST",
            headers: {
              accept: "*/*",
        ...
        if (!forceRefresh) {
          ...
          const allExamsCached = pageExamsUnfiltered.every((exam) =>
            examExists(exam, cachedExams),
          );

          if (allExamsCached && pageExamsUnfiltered.length > 0) {
            consecutiveCachedPages++;
            console.log(
              `Page ${pageNumber}: All exams already cached (${consecutiveCachedPages} consecutive cached pages)`,
            );

            // If we've seen 2 consecutive pages that are fully cached, stop fetching
            if (consecutiveCachedPages >= 2) {
              console.log(
                "✅ All new exams fetched (2 consecutive cached pages detected)",
              );
              hasMorePages = false;
              break;
            }
            ...
            const newExams = pageExamsUnfiltered.filter(
              (exam) => !examExists(exam, allExamsUnfiltered),
            );
            allExamsUnfiltered.push(...newExams);
            console.log(
              `Page ${pageNumber}: Found ${newExams.length} new exams`,
            );
          }
        } else {
          ...
          console.log(
            `Page ${pageNumber}: Found ${pageExamsUnfiltered.length} exams`,
          );
        }
    ...
    allExamsFiltered.push(
      ...parseExamCards(
        allExamsUnfiltered
          .map((exam) => `<div class="exam-item">${JSON.stringify(exam)}</div>`)
          .join(""),
        false,
      ),
    );
